...
import { FONT_OPTIONS, TEXT_COLOR_OPTIONS, PAGE_BACKGROUNDS } from "@/lib/fontMap";
...
export default function SpreadEditorPage({ params }: { params: { id: string; spreadId: string } }) {
  ...
  function setBackground(side: Side, background: JournalPage["background"]) {
    updatePage(side, (page) => ({ ...page, background }));
  }
  ...
  return (
    <main className="min-h-screen flex flex-col items-center py-8 px-4">
      ...
      <div className="w-full max-w-5xl flex justify-between mb-3 px-1">
        <BackgroundPicker value={spread.left.background} onChange={(b) => setBackground("left", b)} />
        <BackgroundPicker value={spread.right.background} onChange={(b) => setBackground("right", b)} />
      </div>
      ...
      {/* the open book */}
      <div className="w-full max-w-5xl bg-[#5E4028] rounded-sm shadow-spread p-3 md:p-5 flex gap-[2px] relative">
        <PageArea
          side="left"
          page={spread.left}
...
function PageArea({
  ...
  return (
    <div
      ...
      className={`relative flex-1 aspect-[3/4] overflow-hidden ${bg} ${
...
function BackgroundPicker({
  ...
  value: JournalPage["background"];
  onChange: (b: JournalPage["background"]) => void;
}) {
  return (
    <div className="flex gap-1.5">
      {PAGE_BACKGROUNDS.map((b) => (
        <button
          key={b.key}
          onClick={() => onChange(b.key as JournalPage["background"])}
          title={b.label}
          className={`w-6 h-6 rounded-full border-2 ${b.className} ${
            value === b.key ? "border-rust" : "border-transparent"
          }`}
        />
      ))}
    </div>
